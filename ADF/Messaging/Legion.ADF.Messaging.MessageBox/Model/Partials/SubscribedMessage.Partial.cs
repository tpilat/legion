...
{
	internal static IResult<SubscribedMessage?> Create(
		...
		DTOs.MessageBoxMessageDto messageDto)
	{
		...
		var result = new ResultBuilder<SubscribedMessage?>();
		...
		var validationResult =
			DefaultDBValidator
				.Validate(subscribedMessage);

		if (result.MergeHasError(scopeContext, validationResult, true))
			return result.Build();

		return result.WithData(subscribedMessage).Build();
	}
