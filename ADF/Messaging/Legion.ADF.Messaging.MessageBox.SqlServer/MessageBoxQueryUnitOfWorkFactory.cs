...
public partial class MessageBoxQueryUnitOfWorkFactory : IMessageBoxQueryUnitOfWorkFactory, Legion.Model.Repositories.IQueryUnitOfWorkFactory<IMessageBoxQueryUnitOfWork>
{
	public IMessageBoxQueryUnitOfWork Create(Legion.Database.IConnectionProvider connectionProvider)
	{
		...
		return new MessageBoxQueryUnitOfWork(efConnectionProvider);
	}

	public IMessageBoxQueryUnitOfWork Create(Legion.Model.Repositories.IUnitOfWork unitOfWork)
	{
		...
		return new MessageBoxQueryUnitOfWork(dbUnitOfWork);
	}

	public IMessageBoxQueryUnitOfWork Create(Legion.Model.Repositories.IQueryUnitOfWork queryUnitOfWork)
	{
		...
		return new MessageBoxQueryUnitOfWork(dbQueryUnitOfWork);
	}

	public IMessageBoxQueryUnitOfWork Create(
		IServiceProvider serviceProvider,
		string connectionStirng,
		System.Data.IsolationLevel? isolationLevel,
		bool? allowLocking,
		bool createAuditEntryStore)
		=> new MessageBoxQueryUnitOfWork(
			serviceProvider,
			connectionStirng,
			isolationLevel,
			allowLocking,
	...
	public IMessageBoxQueryUnitOfWork CreateWithoutTransaction(
		IServiceProvider serviceProvider,
		string connectionStirng,
		bool? allowLocking,
		bool createAuditEntryStore)
