...
public class SetUp
{
	...
	public static string ConncetionString => "Host=localhost;Database=legion_adf_msg;Port=5432;Username=postgres;Password=postgres;Timeout=120;CommandTimeout=120;MaxPoolSize=20;ApplicationName=Legion.ADF.Messaging.Outbox.IntegrationTests";
	...
	private static async Task<IServiceScopeFactory> GetScopeFactoryAsync()
	{
		...
		using (await _servicesLock.LockAsync())
		{
			...
			services.AddADFMessaging(_configuration)
				...
				{
					outbox
						.AddOutboxMessageType<TestMessage>(scopeContext)
						.AddOutboxMessageType(scopeContext, "myString", "myString", "myString")
						.AddOutboxQueue<Events.Test1QueueOutboxMessageReceivedEvent>(
							scopeContext,
							queueName: "TestQueue",
							factory: msg => new Events.Test1QueueOutboxMessageReceivedEvent(msg),
						...
						.AddOutboxQueue<Events.Test2QueueOutboxMessageReceivedEvent>(
							scopeContext,
							queueName: "FakeQueue",
							factory: msg => new Events.Test2QueueOutboxMessageReceivedEvent(msg),
							timeoutForMessageProcessing: TimeSpan.FromMicroseconds(1),
							isSequentialFIFO: false,
							messagesBatchCount: 1,
							maxDegreeOfParallelism: 1,
							maxMessageProcessingRetryCount: 5,
							messageTypeNamespace: "myString")
						.AddOutboxQueue<Events.Test3QueueOutboxMessageReceivedEvent>(
							scopeContext,
							queueName: "NoHandlerQueue",
							factory: msg => new Events.Test3QueueOutboxMessageReceivedEvent(msg),
							timeoutForMessageProcessing: TimeSpan.FromMicroseconds(1),
							isSequentialFIFO: false,
							messagesBatchCount: 1,
							maxDegreeOfParallelism: 1,
