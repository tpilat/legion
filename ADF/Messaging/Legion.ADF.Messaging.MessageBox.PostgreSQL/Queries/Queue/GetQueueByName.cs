...
public class GetQueueByName :
	QueryDefinition<
		...
		IGetQueueByName
{
	public GetQueueByName(
		IEFConnectionProvider connectionProvider,
		GetQueueByNameQuery getQueueByName)
		: base(connectionProvider, getQueueByName)
	{
	}
	...
	public override async Task<MessageBox.Model.Queue?> ToResultAsync(
	...
	public async Task<Guid?> GetIdQueueAsync(
		IScopeContext scopeContext,
		CancellationToken cancellationToken = default)
	{
		scopeContext = ScopeContext.Create(scopeContext);
		...
		return await GetQuery(scopeContext)
			.Select(iq => iq.IdQueue)
			.FirstOrDefaultAsync(cancellationToken);
	}

	public Guid? GetIdQueue(IScopeContext scopeContext)
	{
		scopeContext = ScopeContext.Create(scopeContext);
