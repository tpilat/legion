...
public partial class VwHostRepository : Legion.ADF.ServiceBus.Hosts.SqlServer.HostsQueryRepositoryBase, Legion.ADF.ServiceBus.Hosts.IHostsQueryRepository<Legion.ADF.ServiceBus.Hosts.Model.VwHost>, Legion.ADF.ServiceBus.Hosts.Model.Repositories.IVwHostRepository
{
	private readonly Lazy<Legion.ACL.IAccessControlManager<Legion.ADF.ServiceBus.Hosts.Model.VwHost>?> _accessControlManager;

	private Legion.ADF.ServiceBus.Hosts.SqlServer.IHostsQueryDbContext? _context;
	...
	{
		var queryable = (_context ??= ConnectionProvider.GetOrCreateDbContext<Legion.ADF.ServiceBus.Hosts.SqlServer.IHostsQueryDbContext>(scopeContext)).VwHost;
