...
internal partial class EnterpriseServiceBus : BackgroundService, IDisposable
{
	...
	private async Task<TimeSpan> StartHostAsync(
	...
	private async Task HostHeartbeatAsync(
		IScopeContext originalScopeContext,
		TimeSpan firstHearbeatDelay,
		CancellationToken cancellationToken = default)
	{
		await Task.Delay(firstHearbeatDelay, cancellationToken);
				...
				else
				{
					...
					else
					{
						...
						else
						{
							...
							await CheckDistributedCacheAsync(
								scopeContext,
		...
		CancellationToken cancellationToken = default)
	{
		...
		var set = await SimplePersistentCache.SetValuePermanentlyAsync("my key", "my value", cancellationToken);
	}
