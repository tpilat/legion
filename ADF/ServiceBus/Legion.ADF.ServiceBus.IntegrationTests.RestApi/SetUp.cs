using Legion.ADF.ServiceBus.RestApi.Client;
...
public class SetUp
{
	...
	public static string ConncetionString => "Host=localhost;Database=legion_adf_servicebus;Port=5432;Username=postgres;Password=postgres;Timeout=120;CommandTimeout=120;MaxPoolSize=20;ApplicationName=Legion.ADF.ServiceBus.IntegrationTests";
	...
	private static async Task<IServiceScopeFactory> GetScopeFactoryAsync()
	{
		...
		using (await _servicesLock.LockAsync())
		{
			...
			services.AddServiceBusRestApiClient("ServiceBusRestApi");

			//services.AddInMemoryMessageBus([typeof(RestApi.Client.ServiceBusRestApiClient).Assembly, typeof(SetUp).Assembly]);
