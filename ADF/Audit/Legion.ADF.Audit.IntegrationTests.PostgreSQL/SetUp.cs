using Legion.ADF.Audit.Extensions;
...
public class SetUp
{
	...
	public static string ConncetionString => "Host=localhost;Database=legion_adf_audit;Port=5432;Username=postgres;Password=postgres;Timeout=120;CommandTimeout=120;MaxPoolSize=20;ApplicationName=Legion.ADF.Audit.IntegrationTests";
	...
	public async Task RunBeforeAnyTests()
	{
		...
		_configuration = new ConfigurationBuilder()
			...
			.AddConfigurationData(new Dictionary<string, string?>
				{
					{
						"ADFAudit:DBSettings:DbConnectionSettings:ADF_AUDIT:ConnectionString",
						ConncetionString
					},
					{
	...
	private static async Task<IServiceScopeFactory> GetScopeFactoryAsync()
	{
		...
		using (await _servicesLock.LockAsync())
		{
			...
			services.AddADFAudit(_configuration)
				.ConfigurePostgreSQL();
