...
public partial class AuthQueryUnitOfWorkFactory : IAuthQueryUnitOfWorkFactory, Legion.Model.Repositories.IQueryUnitOfWorkFactory<IAuthQueryUnitOfWork>
{
	public IAuthQueryUnitOfWork Create(Legion.Database.IConnectionProvider connectionProvider)
	{
		...
		return new AuthQueryUnitOfWork(efConnectionProvider);
	}

	public IAuthQueryUnitOfWork Create(Legion.Model.Repositories.IUnitOfWork unitOfWork)
	{
		...
		return new AuthQueryUnitOfWork(dbUnitOfWork);
	}

	public IAuthQueryUnitOfWork Create(Legion.Model.Repositories.IQueryUnitOfWork queryUnitOfWork)
	{
		...
		return new AuthQueryUnitOfWork(dbQueryUnitOfWork);
	}

	public IAuthQueryUnitOfWork Create(
		IServiceProvider serviceProvider,
		string connectionStirng,
		System.Data.IsolationLevel? isolationLevel,
		bool? allowLocking,
		bool createAuditEntryStore)
		=> new AuthQueryUnitOfWork(
			serviceProvider,
			connectionStirng,
			isolationLevel,
			allowLocking,
	...
	public IAuthQueryUnitOfWork CreateWithoutTransaction(
		IServiceProvider serviceProvider,
		string connectionStirng,
		bool? allowLocking,
		bool createAuditEntryStore)
