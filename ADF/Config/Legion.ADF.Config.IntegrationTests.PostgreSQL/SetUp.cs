...
public class SetUp
{
	...
	public static string ConncetionString => "Host=localhost;Database=legion_adf_conf;Port=5432;Username=postgres;Password=postgres;Timeout=120;CommandTimeout=120;MaxPoolSize=20;ApplicationName=Legion.ADF.Config.IntegrationTests";
	...
	public async Task RunBeforeAnyTests()
	{
		...
		_configuration = new ConfigurationBuilder()
			...
			.AddConfigurationData(new Dictionary<string, string?>
				{
					{
						"ADFConfig:DBSettings:DbConnectionSettings:ADF_CONFIG:ConnectionString",
						ConncetionString
					},
					{
	...
	private static async Task<IServiceScopeFactory> GetScopeFactoryAsync()
	{
		...
		using (await _servicesLock.LockAsync())
		{
			...
			services.AddADFConfig(_configuration)
				.ConfigurePostgreSQL();
			...
			services.AddInMemoryMessageBus([typeof(Config.PostgreSQL.TableInfoProvider).Assembly, typeof(SetUp).Assembly]);
