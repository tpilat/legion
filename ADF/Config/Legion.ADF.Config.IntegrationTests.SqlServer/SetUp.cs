...
public class SetUp
{
	...
	public static string ConncetionString => "Server=127.0.0.1;Database=legion_adf_conf;User Id=sa;Password=Password1.;TrustServerCertificate=True;MultipleActiveResultSets=true";
	...
	private static async Task<IServiceScopeFactory> GetScopeFactoryAsync()
	{
		...
		using (await _servicesLock.LockAsync())
		{
			...
			services.AddADFMessaging(_configuration)
				.AddDomainEvents(domainEvents => domainEvents.ConfigureDomainEventsSqlServer());

			services.AddInMemoryMessageBus([typeof(Config.SqlServer.TableInfoProvider).Assembly, typeof(SetUp).Assembly]);
