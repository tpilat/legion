...
public partial class CacheUnitOfWorkFactory : ICacheUnitOfWorkFactory
{
	public ICacheUnitOfWork Create(Legion.Database.IConnectionProvider connectionProvider)
	{
		...
		return new CacheUnitOfWork(efConnectionProvider);
	}

	public ICacheUnitOfWork Create(Legion.Model.Repositories.IUnitOfWork unitOfWork)
	{
		...
		return new CacheUnitOfWork(dbUnitOfWork);
	}

	public ICacheUnitOfWork Create(Legion.Model.Repositories.IQueryUnitOfWork queryUnitOfWork)
	{
		...
		return new CacheUnitOfWork(dbQueryUnitOfWork);
	}

	public ICacheUnitOfWork Create(
	...
	public ICacheUnitOfWork CreateWithoutTransaction(
		IServiceProvider serviceProvider,
		string connectionStirng,
		bool? allowLocking,
		bool createAuditEntryStore)
