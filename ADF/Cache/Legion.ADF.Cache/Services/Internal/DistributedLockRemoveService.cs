...
/// </summary>
public class DistributedLockRemoveService : BackgroundService
{
	private readonly DistributedLockRemoveServiceOptions _options;
	private readonly IServiceProvider _serviceProvider;
	protected readonly IConnectionProviderFactory _connectionProviderFactory;
	private readonly ILogger<DistributedLockRemoveService> _logger;

	public DistributedLockRemoveService(
		IOptions<DistributedLockRemoveServiceOptions> options,
		IServiceProvider serviceProvider,
		IConnectionProviderFactory connectionProviderFactory,
		ILogger<DistributedLockRemoveService> logger)
	{
	...
	{
		var scopeContextGlobal = ScopeContext.Create(nameof(DistributedLockRemoveService));

		while (!cancellationToken.IsCancellationRequested)
		{
			var scopeContext = ScopeContext.Create(scopeContextGlobal, correlationId: GlobalContext.Instance.NewGuid());

			_logger.LogTraceMessage(scopeContext, x => x.InternalMessage($"{nameof(DistributedLockRemoveService)}.{nameof(ExecuteAsync)}: START"));
			...
			try
			{
				...
				await uow.DistributedLockRepository.DeleteExpiredAsync(scopeContext, cancellationToken);
			}
			catch (Exception ex)
			{
				_logger.LogErrorMessage(scopeContext, Exceptions.Internal.ErrorCodes.DistributedLockRemoveService.Default, x => x.ExceptionInfo(ex));
			}
			finally
			{
