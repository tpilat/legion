...
public class BatchLogMessageStore : BatchWriter<ILogMessage>, IDisposable
{
	private readonly ITableInfoBulkInsert<Model.Log> _tableInfoBulkInsert;

	public BatchLogMessageStore(
		ConnectionStringProvider connectionStringProvider,
		ITableInfoProvider tableInfoProvider,
		ITableInfoBulkInsertFactory tableInfoBulkInsertFactory,
		IOptions<BatchLogMessageStoreOptions> options)
		: base(options?.Value, errorLogger: null)
	{
		...
		_tableInfoBulkInsert = tableInfoBulkInsertFactory.Create<Model.Log>(tableInfoProvider, log => log.ToDictionary(), connectionString);
	}

	protected override Task<ulong> WriteBatchAsync(
		IEnumerable<ILogMessage> batch,
		CancellationToken cancellationToken = default)
	{
		if (batch == null || batch.Any() == false)
			return Task.FromResult((ulong)0);

		var scopeContext = ScopeContext.Create(nameof(BatchLogMessageStore));

		var entities = batch.Select(logMessage =>
		{
			var createResult = Model.Log.CreateLog(scopeContext, logMessage, null);
