...
public class SetUp
{
	...
	public static string ConncetionString => "Host=localhost;Database=legion_adf_logs;Port=5432;Username=postgres;Password=postgres;Timeout=120;CommandTimeout=120;MaxPoolSize=20;ApplicationName=Legion.ADF.Logs.IntegrationTests";
	...
	private static async Task<IServiceScopeFactory> GetScopeFactoryAsync()
	{
		...
		using (await _servicesLock.LockAsync())
		{
			...
			services.AddInMemoryMessageBus([typeof(Logs.PostgreSQL.TableInfoProvider).Assembly, typeof(SetUp).Assembly]);
