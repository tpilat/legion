...
internal class PostgreSQLConnectionProvider : EFConnectionProvider, IEFConnectionProvider, IConnectionProvider, IDisposable, IAsyncDisposable
{
	public override NpgsqlConnection? GetDbConnection()
	{
		if (DbConnection == null)
			return null;

		if (DbConnection is NpgsqlConnection conn)
			return conn;
	...
	public override NpgsqlConnection GetOrCreateNewDbConnection(out bool isNewConnection)
	{
	...
	public override NpgsqlConnection CreateNewDbConnection()
	{
		if (string.IsNullOrWhiteSpace(ConnectionString))
			Throw.InvalidOperationException(Legion.EntityFrameworkCore.Exceptions.Internal.ErrorCodes.DbContext.NoConnectionString);
	...
	private PostgreSQLConnectionProvider(
	...
	public override void OnConfiguring(DbContextOptionsBuilder optionsBuilder)
	{
		if (DbConnection == null)
		{
			...
			optionsBuilder.UseNpgsql(string.Format(ConnectionString, System.Environment.GetEnvironmentVariable("PGPASSWORD")));
		}
		else
		{
		...
		where TConnectionStringProvider : class, IConnectionStringProvider
	{
		...
		var connectionProvider = new PostgreSQLConnectionProvider(
			serviceProvider,
			null,
