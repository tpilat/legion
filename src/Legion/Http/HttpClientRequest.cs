...
public class HttpClientRequest : IHttpClientRequest
{
	...
	public static HttpClientRequest FromHttpRequest(HttpRequestMessage httpRequestMessage, bool setHeaders)
	{
		if (httpRequestMessage == null)
			throw new ArgumentNullException(nameof(httpRequestMessage));

		var result = new HttpClientRequest
		{
		...
		else if (httpRequestMessage.Content is JsonContentNet jsonContent)
		{
		...
		else if (httpRequestMessage.Content is MultipartContent multipartContent)
		{
			//TODO dorobit
	...
	internal static Task<HttpClientRequest> FromHttpRequestWithContentAsync(HttpRequestMessage httpRequestMessage, bool setHeaders)
		=> FromHttpRequest(httpRequestMessage, setHeaders).ReadContentAsync();

	internal async Task<HttpClientRequest> ReadContentAsync()
	{
					...
					foreach (var streamContent in StreamContents)
					{
						if (string.IsNullOrWhiteSpace(streamContent.HttpContentNameFormDataMultipartPurposeMultipartPurpose))
							...
							multipartFormDataContent.Add(
								streamContent.ToStreamContent(),
								streamContent.HttpContentNameFormDataMultipartPurposeMultipartPurpose);
						else
							multipartFormDataContent.Add(
								streamContent.ToStreamContent(),
								streamContent.HttpContentNameFormDataMultipartPurposeMultipartPurpose,
								streamContent.HttpContentFileNameFormDataMultipartPurposeMultipartPurpose);
					}
				...
				if (ByteArrayContents != null)
					foreach (var byteArrayContent in ByteArrayContents)
					{
						if (string.IsNullOrWhiteSpace(byteArrayContent.HttpContentNameFormDataMultipartPurposeMultipartPurpose))
							...
							multipartFormDataContent.Add(
								byteArrayContent.ToByteArrayContent(),
								byteArrayContent.HttpContentNameFormDataMultipartPurposeMultipartPurpose);
						else
							multipartFormDataContent.Add(
								byteArrayContent.ToByteArrayContent(),
								byteArrayContent.HttpContentNameFormDataMultipartPurposeMultipartPurpose,
								byteArrayContent.HttpContentFileNameFormDataMultipartPurposeMultipartPurpose);
					}
