...
public interface IRequestBuilder<TBuilder, TObject>
	...
	where TObject : IHttpClientRequest
{
	...
	TBuilder Method(System.Net.Http.HttpMethod httpMethod, bool force = true);
	...
	TBuilder AddItem(string key, object? value, bool force = false);
	...
	where TObject : IHttpClientRequest
{
	...
	public TBuilder Method(System.Net.Http.HttpMethod httpMethod, bool force = true)
	{
		if (force || string.IsNullOrWhiteSpace(_request.HttpMethod))
		{
			if (httpMethod == null)
	...
	public TBuilder AddItem(string key, object? value, bool force = false)
	{
		...
		{
			_request.Items ??= [];
			_request.Items[key] = value;
		}
		else
		{
			_request.Items ??= [];
			_request.Items.TryAdd(key, value);
		}
...
public class RequestBuilder : RequestBuilderBase<RequestBuilder, IHttpClientRequest>
{
	public RequestBuilder()
		: this(new HttpClientRequest())
	{
	}

	public RequestBuilder(IHttpClientRequest request)
		: base(request)
	{
	}

	public static implicit operator HttpClientRequest?(RequestBuilder builder)
	{
		if (builder == null)
			return null;

		return builder._request as HttpClientRequest;
	}

	public static implicit operator RequestBuilder?(HttpClientRequest request)
	{
		if (request == null)
			return null;
