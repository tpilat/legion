...
public class HttpContentDto
{
	...
	private bool _contentWasRead;
	private readonly AsyncLock _readContentLock = new();
	public async Task<HttpContentDto> ReadContentAsync()
	{
		if (_contentWasRead)
			return this;

		using (await _readContentLock.LockAsync().ConfigureAwait(false))
		{
			if (_contentWasRead)
				return this;
