...
namespace Legion.Http.Json;

public sealed partial class JsonContentNet : System.Net.Http.HttpContent
{
	...
	private JsonContentNet(object? inputValue, Type inputType, MediaTypeHeaderValue? mediaType, JsonSerializerSettings? options)
	{
		if (inputType == null)
	...
	public static JsonContentNet Create<T>(T inputValue, MediaTypeHeaderValue? mediaType = null, JsonSerializerSettings? options = null)
		=> Create(inputValue, typeof(T), mediaType, options);

	public static JsonContentNet Create(object? inputValue, Type inputType, MediaTypeHeaderValue? mediaType = null, JsonSerializerSettings? options = null)
	...
	private async Task SerializeToStreamAsyncCoreAsync(Stream targetStream, CancellationToken cancellationToken)
	{
		...
		if (targetEncoding != null && targetEncoding != Encoding.UTF8 && targetEncoding != GlobalCache.UTF8NoBOM)
		{
			using var transcodingStream = new TranscodingWriteStreamNet(targetStream, targetEncoding);
			JsonSerializerHelper.Serialize(Value, ObjectType, transcodingStream, _jsonSerializerSettings);
		...
		{
			using var streamWriter = new StreamWriter(targetStream, targetEncoding, 1024, true);
			JsonSerializerHelper.Serialize(Value, ObjectType, streamWriter, _jsonSerializerSettings);
		}
	}
