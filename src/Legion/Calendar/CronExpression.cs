...
public sealed class CronExpression : IEquatable<CronExpression>
{
	...
	{
		Throw.IfArgumentNull(nameof(expression));
	...
	public DateTime? GetNextOccurrence(DateTime fromUtc, bool inclusive = false)
	{
		if (fromUtc.Kind != DateTimeKind.Utc)
			ThrowWrongDateTimeKindException(nameof(fromUtc));
	...
	{
		if (fromUtc > toUtc)
			ThrowFromShouldBeLessThanToException(nameof(fromUtc), nameof(toUtc));
	...
	{
		if (from > to)
			ThrowFromShouldBeLessThanToException(nameof(from), nameof(to));
...
#pragma warning restore IDE0070 // Use 'System.HashCode'
	...
	{
		CronFormatException.Throw(null, $"{field}: {string.Format(format, args)}", null);
	}

	[MethodImpl(MethodImplOptions.NoInlining)]
	private static void ThrowFormatException(string format, params object[] args)
	{
		CronFormatException.Throw(null, string.Format(format, args), null);
	}

	[MethodImpl(MethodImplOptions.NoInlining)]
	private static void ThrowFromShouldBeLessThanToException(string fromName, string toName)
	{
		ArgException.Throw(fromName, null, $"The value of the {fromName} argument should be less than the value of the {toName} argument.", null);
	}

	[MethodImpl(MethodImplOptions.NoInlining)]
	private static void ThrowWrongDateTimeKindException(string paramName)
	{
		ArgException.Throw(paramName, null, "The supplied DateTime must have the Kind property set to Utc", null);
	}
