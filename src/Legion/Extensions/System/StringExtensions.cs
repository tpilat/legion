...
public static class StringExtensions
{
	...
	{
		encoding ??= GlobalCache.UTF8NoBOM;
		byte[] toEncodeAsBytes = encoding.GetBytes(text);
		string returnValue = Convert.ToBase64String(toEncodeAsBytes);
		return returnValue;
	}

	public static string DecodeFromBase64(this string base64Text, Encoding? encoding = null)
	{
		encoding ??= GlobalCache.UTF8NoBOM;
		byte[] encodedDataAsBytes = Convert.FromBase64String(base64Text);
	...
	public static string? DefaultIfNullOrEmpty(this string text, string? defaultString = null)
		=> string.IsNullOrEmpty(text) ? defaultString : text;
	...
	public static string? DefaultIfNullOrWhiteSpace(this string text, string? defaultString = null)
		=> string.IsNullOrWhiteSpace(text) ? defaultString : text;
	...
	public static double GetSyntacticMatchingScore(this string s, string text, bool ignoreCase, bool ignoreAccent, CultureInfo? ignoreCaseCultureInfo, int searchRange)
	{
		if (s == null)
		{
			if (text == null)
	...
	/// </summary>
	public static bool IsSimilarByJaroWinkler(
		this string s,
		string text,
		double similarityThreshold = 0.7,
		bool ignoreCase = true,
		bool ignoreAccent = true,
		CultureInfo? ignoreCaseCultureInfo = null,
		double bonusThreshold = 0.7,
		int? searchRange = 1,
		bool useWinklekBonus = true)
		=> similarityThreshold <= s.GetJaroWinklerDistance(text, ignoreCase, ignoreAccent, ignoreCaseCultureInfo, bonusThreshold, searchRange, useWinklekBonus);
	...
	public static double GetJaroWinklerDistance(
		...
		bool ignoreCase,
		bool ignoreAccent,
		CultureInfo? ignoreCaseCultureInfo,
		double bonusThreshold,
		int? searchRange,
		bool useWinklekBonus)
	{
		if (s == null)
		{
			if (text == null)
	...
	public static string ReplaceAll(this string seed, char[] chars, string replacementString)
		=> chars.Aggregate(seed, (str, cItem) => str.Replace(cItem.ToString(), replacementString));
...
#elif NET6_0_OR_GREATER
		return char.ToUpper(s[0], cultureInfo) + s[1..];
#endif
	}
	...
	{
		if (pattern == null || startIndex < 0)
			return -1;

		if (startIndex == 0)
			return IndexOfFast(source, pattern);
		...
		if (source.Length <= startIndex + pattern.Length)
			return -1;
	...
	public static string? ExtractBetween(this string searchString, string leftString, string rightString)
		=> ExtractBetween(searchString, leftString, rightString, 0, out _, out _);

	public static string? ExtractBetween(this string searchString, string leftString, string rightString, int startIndex)
		=> ExtractBetween(searchString, leftString, rightString, startIndex, out _, out _);
	...
	public static string BetweenInclude(this string searchString, string leftString, string rightString)
		=> string.Concat(leftString, searchString.ExtractBetween(leftString, rightString), rightString);
	...
	public static string ToCultureSafeDecimalString(this string text, string defaultSeparator = ".")
	{
		...
		var separator = MathHelper.GetDecimalSeparator();
		if (separator != null)
	...
	public static List<string>? ToList(this string s, string delimiter)
	{
		...
		var items = s.Split(new string[] { delimiter }, StringSplitOptions.None);
	...
	public static string ReplaceFirst(this string text, string left, string right, bool replaceFirstRight, string replceWith, bool replaceLeftRight)
		=> ReplaceOne(text, left, right, replaceFirstRight, replceWith, replaceLeftRight, 0, out _);
	...
	{
		if (startIndex < 0) startIndex = 0;
		return ReplaceOne(text, left, right, replaceFirstRight, replceWith, replaceLeftRight, startIndex, out _);
	}
			...
			else
			{
				...
				if (!string.IsNullOrEmpty(right))
				{
					...
					else
					{
						...
						string str1 = replaceLeftRight
#if NETSTANDARD2_0 || NETSTANDARD2_1
							? text.Substring(0, leftIndexBegin)
							: text.Substring(0, leftIndexEnd + 1);
#elif NET6_0_OR_GREATER
							? text[..leftIndexBegin]
							: text[..(leftIndexEnd + 1)];
#endif

						lastIndex = replaceLeftRight
							? rightIndexEnd + 1
							: rightIndexBegin;
	...
	public static List<CharInfo>? GetCharInfos(this string text)
		=> StringHelper.GetCharInfos(text);

	public static string TrimPrefix(this string text, string prefix, bool ignoreCase = false)
		=> StringHelper.TrimPrefix(text, prefix, ignoreCase);

	public static string TrimPostfix(this string text, string postfix, bool ignoreCase = false)
		=> StringHelper.TrimPostfix(text, postfix, ignoreCase);

	public static bool TryTrimPrefix(this string text, string prefix, out string? trimmedText, bool ignoreCase = false)
	{
		trimmedText = StringHelper.TrimPrefix(text, prefix, ignoreCase);
		return trimmedText?.Length < text?.Length;
	}

	public static bool TryTrimPostfix(this string text, string postfix, out string? trimmedText, bool ignoreCase = false)
	{
		trimmedText = StringHelper.TrimPostfix(text, postfix, ignoreCase);
		return trimmedText?.Length < text?.Length;
	}

	public static string? TrimLength(this string text, int maxLength, string? postfix = null)
		=> StringHelper.TrimLength(text, maxLength, postfix);
	...
	public static string FirstToLower(this string text, bool allFirstToLower = false)
	{
		...
		else if (allFirstToLower)
		{
			...
			var toLower = new List<char> { char.ToLower(text[0]) };
			int index = 0;
			for (int i = 1; i < text.Length; i++)
			{
				if (up[i] == text[i])
				{
					toLower.Add(char.ToLower(text[i]));
					index = i;
				}
				else
				{
			...
			if (0 < index)
				toLower = toLower.Take(toLower.Count - 1).ToList();
			else
				index++;

			return string.Concat(new string([.. toLower]), text.SubstringSafe(index));
		}
		else
		{
#if NETSTANDARD2_0 || NETSTANDARD2_1
	...
	public static string FirstToLower(this string s, CultureInfo cultureInfo, bool allFirstToLower = false)
	{
		...
		else if (allFirstToLower)
		{
			...
			var toLower = new List<char> { char.ToLower(s[0], cultureInfo) };
			int index = 0;
			for (int i = 1; i < s.Length; i++)
			{
				if (up[i] == s[i])
				{
					toLower.Add(char.ToLower(s[i], cultureInfo));
					index = i;
				}
				else
				{
			...
			return string.Concat(new string([.. toLower]), s.SubstringSafe(index));
		}
		else
		{
#if NETSTANDARD2_0 || NETSTANDARD2_1
...
#elif NET6_0_OR_GREATER
			return $"{char.ToLower(s[0], cultureInfo)}{s[1..]}";
#endif
		}
	}
...
#pragma warning disable L_ArgNullEx_Str // Type or member is obsolete
		Throw.IfArgumentNull(s);
		Throw.IfArgumentNull(delimiter);
#pragma warning restore L_ArgNullEx_Str // Type or member is obsolete
	...
	public static T? ConvertTo<T>(this string value)
		=> StringHelper.ConvertTo<T>(value);
}
