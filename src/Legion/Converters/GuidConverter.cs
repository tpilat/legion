...
namespace Legion.Converters;

public static class GuidConverter
{
	public static readonly Guid Legion_STRING_NAMESPACE = new("F78ACC30-2EA5-4F24-9A9E-7C7917163097");
	...
	/// <returns>A UUID derived from the <see cref="Legion_STRING_NAMESPACE"/> namespace and value.</returns>
	public static Guid ToGuid(string value) =>
		ToGuid(value, Legion_STRING_NAMESPACE, StringUuidVersionHashAlgorithm.SHA1);
	...
	{
		Throw.IfArgumentDefault(namespaceId);
		Throw.IfArgumentNullOrEmpty(value);

		int versionNumber = (int)version;
		Throw.IfNotContainsIn(versionNumber, [3, 5]);

		// convert the name to a sequence of octets (as defined by the standard or conventions of its namespace) (step 3)
		// ASSUME: UTF-8 encoding is always appropriate
		var nameBytes = GlobalCache.UTF8NoBOM.GetBytes(value);
