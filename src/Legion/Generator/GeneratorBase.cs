	...
	private void CreateFile(string filePath, string content)
	{
		if (WriteMode != WriteModes.None)
		{
			...
			if (WriteMode == WriteModes.Overwritte)
				File.WriteAllText(filePath, content, _generateUFT8WithBOM == true ? Encoding.UTF8 : GlobalCache.UTF8NoBOM);
			else if (WriteMode == WriteModes.Append)
				File.AppendAllText(filePath, content, _generateUFT8WithBOM == true ? Encoding.UTF8 : GlobalCache.UTF8NoBOM);
		}
	}

	protected bool IsFileContentDifferent(string fileName, string newContent)
	{
		return !(File.Exists(fileName) && File.ReadAllText(fileName, _generateUFT8WithBOM == true ? Encoding.UTF8 : GlobalCache.UTF8NoBOM) == newContent);
	}
