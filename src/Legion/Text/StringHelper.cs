...
public static class StringHelper
{
	...
	public static string ToCammelCase(string text, bool strictCammelCase = false, bool removeUnderscores = true, bool throwIfEmpty = true)
	{
		...
		if (string.IsNullOrWhiteSpace(normalizedText))
		{
			if (throwIfEmpty || removeUnderscores)
				Throw.IfNullOrWhiteSpace(normalizedText);
			else
				normalizedText = "_";
		}
	...
	public static string? TrimLength(string text, int length, string? postfix = null)
	{
		if (text == null)
			return null;
		...
		{
			Throw.IfArgumentIsLessThan(length, postfix!.Length);

			var newLength = length - postfix.Length;
			if (newLength == 0)
				return postfix;
			else 
...
#pragma warning restore L_ArgNullEx_Str // Type or member is obsolete
	...
	public static string GenerateLiteral(byte[] value)
	{
		ArgNullException.ThrowIfNull(value);
...
#pragma warning restore L_ArgNullEx_Str // Type or member is obsolete
	...
	public static string GenerateLiteral(object value, bool notNullable = false)
	{
		if (value == null || value == DBNull.Value)
			return $"null{(notNullable ? "!" : "")}";
...
//#endif
	...
	public static string? GetFirstNotEmptyString(params string?[] values)
		=> values?.FirstOrDefault(x => !string.IsNullOrEmpty(x));

	public static string? GetFirstNotWhiteSpaceString(params string?[] values)
	...
	public static string? TrimToFitMaxLength(string? text, int maxLength, string? postfix = null)
	{
		if (text == null)
			return null;
		...
		else
		{
			if (maxLength < postfix!.Length)
				Throw.ArgumentOutOfRangeException(postfix, $"{nameof(maxLength)} = {maxLength} < {nameof(postfix)}.{nameof(postfix.Length)} = {postfix.Length}");
