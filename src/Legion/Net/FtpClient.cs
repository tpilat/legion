...
public class FtpClient : IAsyncFtpClient
{
	...
	public async Task<string[]> GetDirectoryListingAsync(string relativeDirectoryPath, string searchPattern, bool combineResultsWithPath = true, CancellationToken cancellationToken = default)
	{
		...
		using var streamReader = new System.IO.StreamReader(response.GetResponseStream());
		string responseString = await streamReader.ReadToEndAsync().ConfigureAwait(true);
		string[] results = responseString.Split(new string[] { "\r\n", "\n" }, StringSplitOptions.RemoveEmptyEntries);
...
#elif NET6_0_OR_GREATER
		await requestStream.WriteAsync(sourceBytes, cancellationToken).ConfigureAwait(false);
#endif
...
#elif NET6_0_OR_GREATER
		await requestStream.WriteAsync(fileContents, cancellationToken).ConfigureAwait(false);
#endif
	...
	private static List<string> SplitDirectoryPath(string direcotryRelativePath)
	{
