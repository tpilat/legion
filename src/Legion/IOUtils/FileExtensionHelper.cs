...
public class FileExtensionHelper
{
	...
	private static readonly Lazy<byte[]> _pdfSignature = new(() => "%PDF"u8.ToArray());
	private static readonly Lazy<List<byte[]>> _jpgSignature = new(() => new List<byte[]>
			{
				new byte[] { 0xFF, 0xD8, 0xFF, 0xE0 },
				new byte[] { 0xFF, 0xD8, 0xFF, 0xE1 },
				new byte[] { 0xFF, 0xD8, 0xFF, 0xE8 },
			});
	private static readonly Lazy<List<byte[]>> _jpegSignature = new(() => new List<byte[]>
			{
				new byte[] { 0xFF, 0xD8, 0xFF, 0xE0 },
				new byte[] { 0xFF, 0xD8, 0xFF, 0xE2 },
				new byte[] { 0xFF, 0xD8, 0xFF, 0xE3 },
			});
	private static readonly Lazy<byte[]> _bmpSignature = new(() => "BM"u8.ToArray());
	private static readonly Lazy<byte[]> _pngSignature = new(() => new byte[] { 0x89, 0x50, 0x4E, 0x47, 0x0D, 0x0A, 0x1A, 0x0A });
