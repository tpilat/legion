...
public class EventSourceAdapter : EventListener, IEventListener
{
	...
	private readonly Func<string, Guid>? _idEventCounterGetter;
	...
	public IDictionary<string, (Guid IdEventCounter, Action<EventCounterData> OnUpdate)>? AllowedCounters { get; } //IDictionary<name, (idEventCounter, onUpdate)>
	public IDictionary<string, (Func<string, Guid> IdEventCounterGetter, Action<EventCounterData> OnUpdate)>? AllowedCountersWithFuncId { get; } //IDictionary<name, (Func<name, idEventCounter>, onUpdate)>
	...
	public EventSourceAdapter(
		string eventSourceName,
		int eventCounterIntervalSec,
		IDictionary<string, (Guid IdEventCounter, Action<EventCounterData> OnUpdate)> allowedCounters,
		EventLevel eventLevel = EventLevel.LogAlways,
		EventKeywords eventKeywords = EventKeywords.All,
		bool autoEnable = true)
		: this(eventSourceName, eventCounterIntervalSec, eventLevel, eventKeywords, false)
	{
		Throw.IfArgumentNull(allowedCounters);
	...
	public EventSourceAdapter(
		string eventSourceName,
		int eventCounterIntervalSec,
		IDictionary<string, (Func<string, Guid> IdEventCounterGetter, Action<EventCounterData> OnUpdate)> allowedCounters,
		EventLevel eventLevel = EventLevel.LogAlways,
		EventKeywords eventKeywords = EventKeywords.All,
		bool autoEnable = true)
		: this(eventSourceName, eventCounterIntervalSec, eventLevel, eventKeywords, false)
	...
	public EventSourceAdapter(
		...
		Func<string, Guid> idEventCounterGetter,
		EventLevel eventLevel = EventLevel.LogAlways,
		EventKeywords eventKeywords = EventKeywords.All,
		bool autoEnable = true)
		: this(eventSourceName, eventCounterIntervalSec, eventLevel, eventKeywords, false)
		...
		: this(eventSourceName, eventCounterIntervalSec, eventLevel, eventKeywords, false)
	{
		...
		if (_onEventCounter == null && _onIncrementingEventCounter == null)
			throw new ArgumentException($"{nameof(onEventCounter)} == null && {nameof(onIncrementingEventCounter)} == null", nameof(onEventCounter));
			...
			false)
	{
		...
		{
			AddOnUpdateEvent(onUpdate);
			_idEventCounterGetter = options.EventSourceAdapterIdEventCounterGetter ?? throw new InvalidOperationException($"{nameof(options)}.{nameof(options.EventSourceAdapterIdEventCounterGetter)} == null");
		}
	...
	public void Update(IDictionary<string, object> payload)
	{
		...
		if (payload.TryGetValue("Name", out object? nameValue) && nameValue is string name)
		{
			...
			{
				(Guid IdEventCounter, Action<EventCounterData> OnUpdate) callback = default;
			...
			{
				(Func<string, Guid> IdEventCounterGetter, Action<EventCounterData> OnUpdate) callback = default;
				if (AllowedCountersWithFuncId != null && !AllowedCountersWithFuncId.TryGetValue(name, out callback))
					return;

				callback.OnUpdate?.Invoke(new EventCounterData(callback.IdEventCounterGetter?.Invoke(name) ?? Guid.Empty, payload));
			}
			else if (_onUpdate != null)
			{
				_onUpdate.Invoke(new EventCounterData(_idEventCounterGetter?.Invoke(name) ?? Guid.Empty, payload));
			}
			else
			{
				if (payload.TryGetValue("CounterType", out object? counterTypeValue) && counterTypeValue is string counterType)
				{
					if (counterType.Equals("Mean", StringComparison.Ordinal))
						_onEventCounter?.Invoke(new EventCounterItem(_idEventCounterGetter?.Invoke(name) ?? Guid.Empty, payload));
					else if (counterType.Equals("Sum", StringComparison.Ordinal))
						_onIncrementingEventCounter?.Invoke(new IncrementingEventCounterItem(_idEventCounterGetter?.Invoke(name) ?? Guid.Empty, payload));
				}
			}
		}
	}
