...
namespace Legion.Reflection.Internal;
...
public class DelegateFactory : IDelegateFactory
{
	...
	{
		Throw.IfArgumentNull(memberInfo);
	...
	public Func<T?, object?> CreateGet<T>(PropertyInfo propertyInfo)
	{
		...
		Throw.IfNull(getMethod); //TODO: message "Property does not have a getter."
	...
	public Action<T?, object?> CreateSet<T>(PropertyInfo propertyInfo)
	{
		...
		Throw.IfNull(setMethod); //TODO message "Property does not have a setter."
	...
	private static Expression EnsureCastExpression(Expression expression, Type targetType, bool allowWidening = false)
	{
		...
		if (targetType.IsValueType)
		{
			...
			{
				MethodInfo toTargetTypeMethod = typeof(Convert).GetMethod("To" + targetType.Name, [typeof(object)])!;
