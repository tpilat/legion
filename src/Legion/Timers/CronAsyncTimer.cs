...
using Legion.Calendar;
using System.Diagnostics;
using System.Runtime.ExceptionServices;
using System.Threading.Tasks.Sources;

namespace Legion.Timers;

public sealed class CronAsyncTimer : IDisposable
{
	...
	public CronAsyncTimer(string expression)
	...
	public CronAsyncTimer(string expression, TimeZoneInfo zone)
	...
	public CronAsyncTimer(CronExpression expression)
		: this(expression, TimeZoneInfo.Utc)
	{
	}
	...
	/// <param name="expression">The cron expression to use. Use <see cref="CronExpression.Parse(string, CronFormat)"/> for non-standard cron expressions.</param>
	/// <param name="zone">The time zone to use.</param>
	public CronAsyncTimer(CronExpression expression, TimeZoneInfo zone)
	{
...
#pragma warning restore IDE0052 // Remove unread private members
		...
		/// <summary>Wait for the next tick of the timer, or for the timer to be stopped.</summary>
		public ValueTask<bool> WaitForNextTickAsync(CronAsyncTimer owner, CancellationToken cancellationToken)
		{
			lock (this)
			{
				if (_activeWait)
