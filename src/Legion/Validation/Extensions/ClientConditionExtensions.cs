...
public static class ClientConditionExtensions
{
	...
	public static IClientConditionDefinition DefaultOrWhiteSpace<T, TProperty>(this ClientCondition<T> clientCondition, Expression<Func<T, TProperty?>> property)
		=> SetClientCondition(clientCondition, Operators.DefaultOrWhiteSpace, property, (TProperty?)typeof(TProperty).GetDefaultNullableValue());

	public static IClientConditionDefinition NotDefaultOrWhiteSpace<T, TProperty>(this ClientCondition<T> clientCondition, Expression<Func<T, TProperty?>> property)
		=> SetClientCondition(clientCondition, Operators.NotDefaultOrWhiteSpace, property, (TProperty?)typeof(TProperty).GetDefaultNullableValue());
	...
	{
		Throw.IfArgumentNull(leftCondition);
		Throw.IfArgumentNull(rightCondition);
