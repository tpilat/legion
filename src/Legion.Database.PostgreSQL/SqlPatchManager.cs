...
public class SqlPatchManager
{
	...
	private bool ConnectToDB()
	{
		...
		try
		{
			...
			if (_connectionString.Contains("{0}"))
				_connectionString = string.Format(_connectionString, System.Environment.GetEnvironmentVariable("PGPASSWORD"));
	...
	private void CreatePatchTable()
	{
		...
		try
		{
			var cmd = new NpgsqlCommand($"SELECT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = '{_dbPatchSchema}' AND table_name = '{_dbPatchTable}')", _connection);
...
";
	...
	private DateTime GetLastPatch()
	{
		...
		try
		{
			var cmd = new NpgsqlCommand($"SELECT {COLUMN_PatchIdentifier} FROM {_dbPatchSchemaTable} ORDER BY {COLUMN_PatchIdentifier} DESC LIMIT 1", _connection);
	...
	private string GetLastVersion(DateTime patchDate)
	{
		...
		try
		{
			var cmd = new NpgsqlCommand($"SELECT {COLUMN_Version} FROM {_dbPatchSchemaTable} WHERE {COLUMN_PatchIdentifier} = @patchDate", _connection);
	...
	private string Patch(DateTime patchDate, string lastVersion, string file, bool loadFromFile)
	{
		...
		try
		{
			...
			using (var tran = _connection!.BeginTransaction())
			{
				try
				{
					var cmd = new NpgsqlCommand(sql, _connection, tran);
					cmd.ExecuteNonQuery();

					cmd = new NpgsqlCommand($"INSERT INTO {_dbPatchSchemaTable} ({COLUMN_PatchIdentifier}, {COLUMN_Version}, {COLUMN_Description}, {COLUMN_Created}, {COLUMN_FilePath}, {COLUMN_Script}, {COLUMN_Hash}) VALUES (@patchIdentifier, @ver, @description, @created, @filePath, @script, @hash)", _connection, tran);
