...
public class DictionaryTableBulkInsert : DictionaryTable, IDisposable
{
	...
	public DictionaryTableBulkInsert(IDictionaryTableOptions options, NpgsqlConnection? connection)
		: base(options, true)
	{
		...
		Func<T, Dictionary<string, object?>> dictionaryMapper)
	{
		...
		ulong result = 0;
		using var writer = dbConnection.BeginBinaryImport(sql);

		foreach (var entity in entities)
		{
			writer.StartRow();
			var entityDict = dictionaryMapper(entity);
	...
	public async Task<ulong> WriteBatchAsync(IEnumerable<IDictionary<string, object?>> rows, bool openConnection = false, bool disposeConnection = false, CancellationToken cancellationToken = default)
	{
		if (rows == null || !rows.Any())
			return 0;
		...
		return await WriteBatchAsync(rows, _connection, openConnection, disposeConnection, cancellationToken);
	}
		...
		using (var writer = connection.BeginBinaryImport(ToCopySql()))
		{
			foreach (var row in rows)
			{
				...
				foreach (var propertyName in PropertyNames)
				{
					...
					await writer.WriteAsync(value, ColumnTypes[propertyName], cancellationToken).ConfigureAwait(false);
				}
			}
