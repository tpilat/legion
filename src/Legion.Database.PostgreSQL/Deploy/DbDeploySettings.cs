...
public class DbDeploySettings
{
	...
	public void Initialize()
	{
		...
		lock (_initLock)
		{
			...
			var builder = new NpgsqlConnectionStringBuilder(ConncetionString);
			AdminUser = builder.Username!;
			...
			builder?
				...
				.WithPropertyError(x => x.ConncetionString, (obj, parent) =>
				{
					if (obj != null)
					{
						try
						{
							var connStringBuilder = new NpgsqlConnectionStringBuilder(obj.ConncetionString);
						}
						catch (Exception ex)
						{
...
{
	public static IServiceCollection AddPostgreSQLDbDeploySettings(
