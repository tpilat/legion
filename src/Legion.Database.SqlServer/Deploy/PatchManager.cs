...
public class PatchManager
{
	...
	private readonly SqlConnection _sqlServerConnection;
	...
	public PatchManager(DbDeploySettings dbDeploySettings, SqlConnectionManager sqlConnectionManager)
	{
		...
		_sqlServerConnection = connection.SqlConnection;
	...
	private void CreatePatchTable()
	{
		...
		try
		{
			var cmd = new SqlCommand($"SELECT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = '{_dbPatchSettings.PatchSchemaName}' AND table_name = '{_dbPatchSettings.PatchTableName}')", _sqlServerConnection);
...
";
					...
					cmd = new SqlCommand(sql, _sqlServerConnection);
					cmd.ExecuteNonQuery();
	...
	private DateTime GetLastPatch()
	{
		...
		try
		{
			var cmd = new SqlCommand($"SELECT {COLUMN_PatchIdentifier} FROM {_dbPatchSchemaTable} ORDER BY {COLUMN_PatchIdentifier} DESC LIMIT 1", _sqlServerConnection);
	...
	private string GetLastVersion(DateTime patchDate)
	{
		...
		try
		{
			var cmd = new SqlCommand($"SELECT {COLUMN_Version} FROM {_dbPatchSchemaTable} WHERE {COLUMN_PatchIdentifier} = @patchDate", _sqlServerConnection);
	...
	public async Task ExecutePatchCommands(SqlFile sqlFile, CancellationToken cancellationToken = default)
	{
		...
		try
		{
			...
			var scripts = SqlFile.SplitSqlStatements(sqlFile.SqlScript!);
			foreach (var script in scripts)
			{
				using var cmd1 = new SqlCommand(script);
			...
			var cmd2 = new SqlCommand($"INSERT INTO {_dbPatchSchemaTable} ({COLUMN_PatchIdentifier}, {COLUMN_Version}, {COLUMN_Description}, {COLUMN_Created}, {COLUMN_FilePath}, {COLUMN_Script}, {COLUMN_Hash}) VALUES (@patchIdentifier, @ver, @description, @created, @filePath, @script, @hash)");
