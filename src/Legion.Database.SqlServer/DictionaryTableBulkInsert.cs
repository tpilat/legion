...
public class DictionaryTableBulkInsert : DictionaryTable, IDisposable
{
	private readonly SqlConnection? _connection;
	private readonly bool _isInternalConnection;
	private bool _internalConnectionIsOpened;

	public DictionaryTableBulkInsert(IDictionaryTableOptions options)
		: this(options, (SqlConnection?)null)
	{
	}

	public DictionaryTableBulkInsert(IDictionaryTableOptions options, string connectionString)
		: this(options, (SqlConnection?)null)
	{
	...
	public DictionaryTableBulkInsert(IDictionaryTableOptions options, SqlConnection? connection)
		: base(options, true)
	{
		...
		Func<T, Dictionary<string, object?>> dictionaryMapper)
	{
		...
		var rows = entities.Select(e => dictionaryMapper(e)).ToList();
	...
	public async Task<ulong> WriteBatchAsync(IEnumerable<IDictionary<string, object?>?>? rows, string connectionString, CancellationToken cancellationToken = default)
	{
		...
		return await WriteBatchAsync(rows, connection, false, false, cancellationToken).ConfigureAwait(false);
	}
	...
	public Task<ulong> WriteBatchAsync(IEnumerable<IDictionary<string, object?>> rows, bool openConnection = false, bool disposeConnection = false, CancellationToken cancellationToken = default)
	{
		...
		Throw.IfNull(_connection); //TODO: message "No DB connection was defined");

		return WriteBatchAsync(rows, _connection, openConnection, disposeConnection, cancellationToken);
	}

	public async Task<ulong> WriteBatchAsync(IEnumerable<IDictionary<string, object?>?> rows, SqlConnection connection, bool openConnection = false, bool disposeConnection = false, CancellationToken cancellationToken = default)
	{
		ulong result = 0;
		...
		//{
		//	if (PropertyTypeMapping.TryGetValue(propertyName, out var sqlType) == true)
		//		dataTable.Columns.Add(propertyName, SqlServerDbTypeHelper.GetCSharpType(sqlType));
		//	else
		...
		//using (var bulkCopy = ToBulkCopy(connection))
		//{
		//	await bulkCopy.WriteToServerAsync(dataTable, cancellationToken).ConfigureAwait(false);
		//}

		var dataReader = new DictionaryTableDataReader(this, rows);
...
/*
 USAGE:
		...
		{
			await bi.WriteBatch(listErr, "Host=localhost;Database=database;Username=name;Password=pwd").ConfigureAwait(false);
		}
 */
...
//	private readonly Dictionary<string, SqlDbType> _propertyTypeMapping;
...
//	private readonly Dictionary<string, SqlDbType> _columnTypes;
...
//	public BulkInsert(DictionaryTableOptions options, SqlConnection connection)
//	{
//		if (options == null)
...
//			_columnTypes = new Dictionary<string, SqlDbType>();
...
//	public async Task WriteBatch(IEnumerable<T> rows, SqlConnection connection, bool openConnection = false, bool disposeConnection = false)
//	{
//		if (rows == null || rows.Count() == 0)
//			return;
...
//	private SqlDbType ConvertType(string memberName)
//	{
//		if (_propertyTypeMapping != null && _propertyTypeMapping.TryGetValue(memberName, out SqlDbType result))
//			return result;
...
//			return SqlDbType.Bytea;

//		else if (underlyingType == typeof(double))
//			return SqlDbType.Double;

//		else if (underlyingType == typeof(int))
//			return SqlDbType.Integer;
...
//			return SqlDbType.TimestampTz;

//		else if (underlyingType == typeof(Guid))
//			return SqlDbType.Uuid;
