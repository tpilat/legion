...
public class TableInfoDescriptor : TypeDescriptor<ModelBase>
{
	...
	public TableInfoDescriptor(ModelBase data, GeneratorContext context)
		: base(data, context)
	{
		Model = Data;

		Name = "TableInfoProvider";
		FileName = $"{Name}.cs";
	...
	public override void Initialize(string modelName, string contextName)
	{
		...
		TableInfoTargetFolder = Path.Combine(CodeGeneratorConfig.Instance.WorkspacePath, Settings.Namespace_EntityContextModelBuilder(modelName));
		TableInfoNamespace = Settings.Namespace_EntityContextModelBuilder(modelName);
	...
	public override ModelResult Generate(string modelName, string contextName)
	{
		...
		var result =
			GeneratorInvoker
				.Generate<TableInfoGenerator>(
					FilePath(modelName, contextName),
					new Dictionary<string, object> { { nameof(TableInfoDescriptor), this }, { nameof(modelName), modelName }, { nameof(contextName), contextName } });
