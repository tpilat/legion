...
internal static class UnboxT<T>
{
	...
	private static Func<object, T?> Create(Type type)
	{
		...
		{
			var nullableFieldMethod = typeof(UnboxT<T>).GetMethod(nameof(UnboxT<T>.NullableField), BindingFlags.NonPublic | BindingFlags.Static);
			var genericMethod = nullableFieldMethod!.MakeGenericMethod([type.GetGenericArguments()[0]])!;
