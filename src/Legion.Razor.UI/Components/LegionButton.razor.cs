...
public partial class LegionButton : LegionComponentBase
{
	...
	/// <summary>
	/// Gets or sets a value indicating whether this <see cref="LegionButton"/> is disabled.
	/// </summary>
	...
	{
		return $"lui-button lui-button-{getButtonSize()} lui-variant-{Enum.GetName(typeof(Variant), Variant).ToLowerInvariant()} lui-{Enum.GetName(typeof(ButtonStyle), ButtonStyle).ToLowerInvariant()} lui-shade-{Enum.GetName(typeof(Shade), Shade).ToLowerInvariant()}{(IsDisabled ? " lui-state-disabled" : "")}{(string.IsNullOrEmpty(Text) && !string.IsNullOrEmpty(Icon) ? " lui-button-icon-only" : "")}";
	}
}
