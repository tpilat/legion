...
public partial class LegionTheme : IDisposable
{
	/// <summary>
	...
	private static readonly string Version = typeof(LegionTheme).Assembly.GetName().Version.ToString();

	private string Href => $"{Path}/{theme}-base.css?v={Version}";

	private string WcagHref => $"{Path}/{theme}-wcag.css?v={Version}";

	private string Path => Embedded ? $"_content/Legion.Blazor/css" : "css";
