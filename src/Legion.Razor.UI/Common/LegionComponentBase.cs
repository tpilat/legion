...
public class LegionComponentBase : ComponentBase, IDisposable
{
	/// <summary>
	...
	/// <summary>
	/// Gets or sets a value indicating whether this <see cref="LegionComponent"/> is visible. Invisible components are not rendered.
	/// </summary>
	...
	private DotNetObjectReference<LegionComponentBase>? reference;

	/// <summary>
	/// Gets the reference for the current component.
	/// </summary>
	/// <value>The reference.</value>
	protected DotNetObjectReference<LegionComponentBase> Reference
	{
		get
		{
			if (reference == null)
	...
	protected override async Task OnAfterRenderAsync(bool firstRender)
	{
		...
		if (firstRender || visibleChanged)
		{
			...
			if (Visible)
			{
				...
				{
					await JSRuntime.InvokeVoidAsync("Legion.addContextMenu", GetId(), Reference);
				}

				if (MouseEnter.HasDelegate)
				{
					await JSRuntime.InvokeVoidAsync("Legion.addMouseEnter", GetId(), Reference);
				}

				if (MouseLeave.HasDelegate)
				{
					await JSRuntime.InvokeVoidAsync("Legion.addMouseLeave", GetId(), Reference);
				}
			}
		}
	}
	...
	[JSInvokable("LegionComponent.RaiseContextMenu")]
	...
	/// </summary>
	[JSInvokable("LegionComponent.RaiseMouseEnter")]
	...
	/// </summary>
	[JSInvokable("LegionComponent.RaiseMouseLeave")]
	...
	public virtual void Dispose()
	{
		...
		if (IsJSRuntimeAvailable)
		{
			if (ContextMenu.HasDelegate)
			{
				JSRuntime.InvokeVoidAsync("Legion.removeContextMenu", UniqueID);
			}

			if (MouseEnter.HasDelegate)
			{
				JSRuntime.InvokeVoidAsync("Legion.removeMouseEnter", UniqueID);
			}

			if (MouseLeave.HasDelegate)
			{
				JSRuntime.InvokeVoidAsync("Legion.removeMouseLeave", UniqueID);
			}
		}
	}
